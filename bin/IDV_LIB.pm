...
sub Chain_Reset{#before we running GT preperation 
	...
	#my $Register_Size = $IDV_Reset_Register->get_size;#For IA Reset
	my $IA_counter=0;
	...
	foreach my $alias (@list_aliases){
		...
		if (!($alias_Name =~ m/(dummy.*hi|DUMMY.*HI|dummy.*HI|DUMMY.*hi|dummy.*msb|DUMMY.*msb|DUMMY.*MSB|dummy.*MSB|dummy.*lo|DUMMY.*LO|dummy.*LO|DUMMY.*lo|dummy.*lsb|DUMMY.*lsb|DUMMY.*LSB|dummy.*LSB)/)){
			$IA_counter=$IA_counter+1;
		}
	}
	foreach my $alias (@list_aliases){
		$alias=$alias->get_name;
		if (!($alias =~ m/(dummy.*hi|DUMMY.*HI|dummy.*HI|DUMMY.*hi|dummy.*msb|DUMMY.*msb|DUMMY.*MSB|dummy.*MSB|dummy.*lo|DUMMY.*LO|dummy.*LO|DUMMY.*lo|dummy.*lsb|DUMMY.*lsb|DUMMY.*LSB|dummy.*LSB)/)){
			if ($alias =~ "reset" or $IA_counter==1 ){
...
sub CAPTURE_COUNTER{ #Need to select the appropriate banks
	...
	if (($#banks + 1) == 2){$IDVCNTR = $CounterALL_Register;}
	elsif($banks[0]=~"A"){$IDVCNTR = $CounterA_Register;}

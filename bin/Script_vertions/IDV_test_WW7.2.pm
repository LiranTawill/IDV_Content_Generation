...
sub IDV_test{
	my ($chain, $osc, $chain_focus,$tap_ratio, $tap_spec,$Fublet_Template_input,$Reset_Register,$CounterA_Register,$CounterB_Register,$CounterALL_Register , $Control_Register,$GT_Config,$parallel) = @_;
	...
	IDV_LIB::Chain_Reset($chain,$Reset_Register,$tap_ratio,$GT_Config,$chain_focus,$tap);
	...
	foreach my $fublet (IDV_LIB::FUBLETS($Control_Register,$tap, %fub)){ #Fublets in chain through TAP spec file or the two folders above. Fub instance name,Only fublets that we use(1 in bypass or 11)
		...
		if (!$fub_contains_osc){
			if ("$fub{$fublet}[0]" =~ "lkg"){
				$noosc_after1fub += 2; #fublet without the OSC that we will shift 5 0s X number of fublets to ignore them
				$noosc_tot += 2;#TOT fublet without the OSC that we will shift 5 0s X number of fublets to ignore them	
			}else{
				$noosc_after1fub += 1;#fublet without the OSC that we will shift 5 0s X number of fublets to ignore them
				$noosc_tot += 1;#TOT fublet without the OSC that we will shift 5 0s X number of fublets to ignore them
			}
		...
		#	pass itpp "expandata:xxTCK, $tap_ratio;";				
			...
			IDV_LIB::WAIT_MEASURE($fublet,$osc,$tap_ratio,%fub); #fublet and osc needed for the label.
			IDV_LIB::CAPTURE_COUNTER($chain,$Control_Register,$CounterA_Register,$CounterB_Register,$CounterALL_Register,$fublet,$osc,$parallel, $chain_focus,@banks); 	#need to fublet and osc to know if there is B, or A and B counters. 
			IDV_LIB::WAIT_AFTER($tap_ratio);
		}
	}
		
	IDV_LIB::FLUSH_STOP_END($chain,$Control_Register,$CounterALL_Register, $chain_focus);
}
1;
